...
def readDataFromFile(path):
    """
    return datasets List<Dict>
    {   words:['i', 'charge', 'it', 'at', 'night', 'and', 'skip', 'taking', 'the', 'cord', 'with', 'me', 'because', 'of']
        labels:['O', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'B-T', 'O', 'O', 'O', 'O', 'O', 'B-P', 'B-T', 'I-T', 'O']
        relations:[[15, 16, 16, 18]]
        relations_gold:[[9, 10, -1, -1], [16, 18, 15, 16]]
    }
    ...
    for l in lines:
        ...
        elif len(l.strip().split("\t")) == 4:  #(op_beg,op_end,ap_beg,ap_end)
            rel = list(map(int, l.strip().split("\t")))
            relations_gold.append([rel[2],rel[3],rel[0],rel[1]]) #(ap_beg,ap_end,op_beg,op_end)
            if -1 not in rel:
                relations.append(rel) #只添加形成关系的AO-pair
...
def convert_examples_to_features(examples, tokenizer, max_seq_length=100):
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for i in range(len(input_ids)):
            relations.append([0]*len(input_ids)) #相当于一个二维矩阵
        ...
        for gr in gold_relations:
            for idx in range(gr[0]+1,gr[1]+1):# +1是因为有CLS
        ...
        features.append(
            InputFeatures(
                tokens, #['[CLS]','it','is','expand','##able']
                input_ids,
                input_mask,
                segment_ids,
                labels, #无padding
                label_ids,# 0为padding值
                relations,#二维矩阵
                gold_relations,#[[3, 4, 4, 5], [13, 14, 9, 10], [37, 38, 9, 10], [17, 19, 9, 10]]
                token_to_orig_map))#{1: 0, 2: 1, 3: 2, 4: 3, 5: 4, 6: 5}
    print(sentlen)     #train:3045      test:800
    print(aspectlen)   #train:2359      test:653
    print(opinionlen)  #train:2500      test:677
    print(relationlen) #train:1535      test:380
    print("\n")
    return features
...
if __name__ == '__main__':
    ...
    ## Required parameters
    parser.add_argument("--train_file", default='data/2014LapStandard.train',type=str)
    # parser.add_argument("--train_file", default='data/2014Lap.small',type=str)
    parser.add_argument("--dev_file", default='data/2014LapStandard.test',type=str)
    parser.add_argument("--test_file", default='data/2014LapStandard.test',type=str)
    parser.add_argument("--output_file", default='data/2014Lap.pt',type=str)
    ...
    #torch.save({"train":train_features, "test":test_features}, args.output_file)
    # print(1)