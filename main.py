...
def relationPredictCheck(relationPredict, batch_relation):
    ...
    pred = torch.gt(pred, relationCheck).data.numpy() # >0.1就认为有关系
...
def bert_load_state_dict(model, state_dict):
    ...
    def load(module, prefix=''):
        ...
        module._load_from_state_dict(
            state_dict=state_dict, prefix=prefix, strict=True, missing_keys=missing_keys, unexpected_keys=unexpected_keys, error_msgs=error_msgs,local_metadata=local_metadata)
...
def read_data(train_features, type, batchsize):
    ...
    all_input_ids = torch.tensor([f.token_ids for f in train_features], dtype=torch.long)#[total_number,max_seq_len]
    all_input_mask = torch.tensor([f.token_mask for f in train_features], dtype=torch.long)#[total_number,max_seq_len]
    all_segment_ids = torch.tensor([f.segmentId for f in train_features], dtype=torch.long)#[total_number,max_seq_len]
    all_relations = torch.tensor([f.relations for f in train_features], dtype=torch.long)#[total_number,max_seq_len,max_seq_len]
    all_labels = torch.tensor([f.label_ids for f in train_features], dtype=torch.long)#[total_number,max_seq_len]
    all_labels[:, :1] = torch.ones(all_labels.size(0), 1).long()
    if type == "train":
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_relations, all_labels) #类似于zip函数
        train_sampler = RandomSampler(train_data) #随机打乱
    else:
