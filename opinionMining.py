...
class opinionMining(nn.Module):
    ...
    def neg_log_likelihood_loss(self, all_input_ids, all_segment_ids, all_labels, all_relations, all_input_mask):
        ...
        all_segment_ids,[batch,seq_len]
        all_labels, [batch,seq_len]
        all_relations,[batch,seq_len,seq_len]
        all_input_mask[batch,seq_len]
        """
        batch_size = all_input_ids.size(0)
        seq_len = all_input_ids.size(1)
        maskTemp1 = all_input_mask.view(batch_size, 1, seq_len).repeat(1, seq_len, 1) #[batch,seq_len,seq_len]
        maskTemp2 = all_input_mask.view(batch_size, seq_len, 1).repeat(1, 1, seq_len) #[batch,seq_len,seq_len]
        maskMatrix = maskTemp1 * maskTemp2 #[batch,seq_len,seq_len]
        ...
        if self.gpu:
            ...
            weight = torch.FloatTensor([0.01, 1.0])
        relation_loss_function = nn.CrossEntropyLoss(weight=weight)
        relationScoreLoss = r_tensor.view(-1, 1)#[b*s*s,1]
        relationlabelLoss = all_relations.view(batch_size * seq_len * seq_len)
        relationScoreLoss = torch.cat([1 - relationScoreLoss, relationScoreLoss], 1) #[b*s*s,2]
    ...
    def mainStructure(self, maskMatrix, all_input_ids, all_segment_ids, steps, all_input_mask):
        ...
        sequence_output = self.dropout(sequence_output)#[b,s,768]

        # T tensor and R tensor
        t_tensor = torch.zeros(batch_size, seq_len, seq_len)#[batch,seq_len,seq_len]
        r_tensor = torch.zeros(batch_size, seq_len, seq_len)#[batch,seq_len,seq_len]
        if self.gpu:
